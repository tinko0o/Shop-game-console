...
exports.addProduct = async (req, res) => {
  try {
    ...
    const { name, manufacturer, type, release_date, description, price, img } =
      req.body;
    if (
      !name ||
      !manufacturer ||
    ...
    res
      .status(500)
      .json({ success: false, message: "Đã xảy ra lỗi khi thêm sản phẩm" });
  }
};
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    await Cart.updateMany(
      ...
      { $pull: { products: { id: req.params.id } } }
    );
    return res
      .status(200)
    ...
    res.status(500).json({
      success: false,
      message: "Đã xảy ra lỗi khi xóa sản phẩm sản phẩm",
    });
  }
};
...
exports.getProduct = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Đã xảy ra sự cố khi xuất thông tin chi tiết sản phẩm",
    });
  }
};
...
exports.getAllProducts = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Đã sảy ra sự cố khi xuất thông tin sản phẩm",
    });
  }
};
...
exports.searchProducts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Đã xảy ra sự cố khi tìm sản phẩm" });
  }
};
...
exports.getTopSalesProducts = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Đã xảy ra lỗi khi xuất những sản phẩm bán chạy",
    });
  }
};
